...
一份写给**初学者**与**面试者**的 Kubernetes 入门纲领：它是什么、干什么用的、分成哪几块、这些块之间怎么协作。第 00 章从「物理机 → 容器」讲起，不预设任何 K8s 经验。有了这份纲领，之后的任何深入你都知道自己站在哪。
...
| 章节 | 内容 |
| :-- | :-- |
| 00 · K8s 是什么 | 解决什么问题、和 Docker 什么关系 |
| 01 · 工作原理 | 四条贯穿 K8s 的基本原理 |
...
- 名词提示：承重名词在主线内首次承重处加一个「📖 名词提示」框（一两句话 + 指向详解章的链接），每名词全主线至多一框；顺带提到的名词只挂链接、不放假
- B 级首次出现链接：顺带名词由 main.js 自动在首次出现处挂链接（术语注册表在 `site.js` 的 `K8C.terms`）；有提示框的名词不进注册表；术语对照表与面试速查两页不做自动链接
